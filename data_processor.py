import pandas as pd
import glob
import os

def carregar_dados():
    # Pega todos os arquivos CSV na pasta 'extratos'
    arquivos = glob.glob("extratos/*.csv")
    if not arquivos:
        return pd.DataFrame()
    
    df_list = []
    for arquivo in arquivos:
        df = pd.read_csv(arquivo)
        df_list.append(df)
    
    dados = pd.concat(df_list, ignore_index=True)
    return dados

def categorizar_despesa(descricao):
    descricao = str(descricao).lower()
    if 'uber' in descricao or 'posto' in descricao:
        return 'Transporte'
    elif 'ifood' in descricao or 'restaurante' in descricao or 'mercado' in descricao:
        return 'Alimentação'
    elif 'netflix' in descricao or 'spotify' in descricao:
        return 'Lazer/Assinaturas'
    elif 'salario' in descricao or 'pix recebido' in descricao:
        return 'Receita'
    else:
        return 'Outros'

def processar_dados(df):
    # Converte coluna de data
    df['Data'] = pd.to_datetime(df['Data'])
    
    # Aplica a categorização automática
    df['Categoria'] = df['Descricao'].apply(categorizar_despesa)
    
    # Cria colunas auxiliares (Mês/Ano)
    df['Mes'] = df['Data'].dt.month_name()
    df['Ano'] = df['Data'].dt.year
    
    return df